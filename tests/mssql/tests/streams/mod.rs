...
const SQL_VALUE: &str = "SELECT @p1";

#[tokio::test]
async fn should_be_able_to_stream_rows() {
...
#[tokio::test]
async fn should_be_able_to_stream_rows_values() {
    let conn = get_conn().await;
    let expected = 44_i64;
...
#[tokio::test]
async fn should_be_able_to_stream_into_models() {
    let conn = get_conn().await;
